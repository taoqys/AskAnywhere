...
- ✅ **终端安全取词**：在 PowerShell / cmd / Windows Terminal 等窗口不会模拟 Ctrl+C，避免未选词时中断正在运行的命令
...
| ↑ / ↓ | 选中文字后切换操作模式（回答问题 / 解释 / 翻译 / 润色 / 自定义） |
| 回车 | 模式选择中直接发送 |
| Ctrl+Enter | 输入框中发送（多行输入用普通回车换行） |
...
- **UI Automation 优先**：浏览器、Office、编辑器等大多数应用可无副作用取词
- **剪贴板兜底**：仅当 UIA 失败且焦点不在终端类窗口时，模拟 Ctrl+C 取词，并在 800ms 后自动恢复原剪贴板
- **未选词判定**：剪贴板内容未发生变化时视为未选中，不会误取旧剪贴板内容
